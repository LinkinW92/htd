...
import com.skeqi.mes.mapper.yp.equipment.maintain.EquipmentMaintainExcelDao;
 ...
 */
@Service
public class EquipmentMaintainExcelServiceImpl implements EquipmentMaintainExcelService {

	@Autowired
	EquipmentMaintainExcelDao dao;
	...
	public String uploadConfig(MultipartFile file) throws Exception {
		...
		if (!"保养名".equals(json1.getString("0")) || !"保养编号".equals(json1.getString("1"))
		...
		for (int i = 1; i < list.size(); i++) {
			...
			if (dao.findConfigCountName(list.get(i).getString("0")) > 0) {
				msgBuffer.append("保养名：" + list.get(i).getString("0") + " 保养名已存在  ");
				continue;
			}

			if (dao.findConfigCountCode(list.get(i).getString("1")) > 0) {
				msgBuffer.append("保养名：" + list.get(i).getString("1") + "  保养编号已存在  ");
				continue;
			}
				...
				if (list.get(i).getString("2").equals(lineList.get(j).getString("lineName"))) {
					...
					for (int j2 = 0; j2 < equipment.size(); j2++) {
						...
						if (j2 == (equipment.size() - 1)) {
							msgBuffer.append("保养名：" + list.get(i).getString("0") + " 设备不存在  ");
							break j;
						}
					}
				}
				if (j == (lineList.size() - 1)) {
					msgBuffer.append("保养名：" + list.get(i).getString("0") + " 产线不存在  ");
				}
			}
			...
			if (configJson.getInteger("equipmentId") != null) {
				...
				if (dao.findConfigByEquipmentId(configJson.getInteger("equipmentId")) > 0) {
					msgBuffer.append("保养名：" + list.get(i).getString("0") + " 已存在保养配置  ");
					continue;
				}

				// 新增保养配置
				if (dao.addConfig(configJson) != 1) {
					msgBuffer.append("保养名：" + list.get(i).getString("0") + " 添加失败  ");
				}
			}
	...
	public String uploadEdition(MultipartFile file) throws Exception {
		...
		JSONObject json1 = list.get(0);
		if (!"保养名".equals(json1.getString("0")) || !"版本号".equals(json1.getString("1"))
		...
		for (int i = 1; i < list.size(); i++) {
			...
			if (edition.getInteger("equipmentId") != null) {
				...
				if (i == (list.size() - 1)) {
					msgBuffer.append("版本号：" + list.get(i).getString("0") + " 保养名不存在  ");
				}
			}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			config: for (int j = 0; j < configLit.size(); j++) {
				...
				if (configJson.getString("configName").equals(list.get(i).getString("0"))) {
					...
					for (int k = 0; k < editionList.size(); k++) {
						...
						if (k == (editionList.size() - 1)) {
							msgBuffer.append("保养项：" + list.get(i).getString("1") + " 版本号不存在  ");
						}
					}
				...
				if (j == (configJson.size() - 1)) {
					msgBuffer.append("保养项：" + list.get(i).getString("0") + " 保养名不存在  ");
				}

			}
			if (item.getString("editionId") != null) {
				if (dao.addItem(item) != 1) {
					msgBuffer.append("保养项：" + list.get(i).getString("3") + " 添加失败  ");
				}
			}
