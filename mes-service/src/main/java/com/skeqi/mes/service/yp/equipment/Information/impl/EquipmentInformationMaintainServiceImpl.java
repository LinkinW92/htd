...
import com.skeqi.mes.mapper.yp.equipment.Information.EquipmentInformationMaintainDao;
 ...
 */
@Service
public class EquipmentInformationMaintainServiceImpl implements EquipmentInformationMaintainService {

	@Autowired
	EquipmentInformationMaintainDao dao;
	...
	public void add(JSONObject json) throws Exception {
		...
		// 新增
		if (dao.add(json) != 1) {
			throw new Exception("生成保养记录失败");
		}

		// 通过设备id查询出需要点检的点检项
		List<JSONObject> itemList = dao.itemList(equipmentId);
		if (itemList == null || itemList.size() == 0) {
			throw new Exception("选择的设备未配置保养项");
		}
