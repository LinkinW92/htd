...
import com.skeqi.mes.mapper.yp.wms.WmsScrapIssueDao;
 ...
 */
@Service
public class WmsScrapIssueServiceImpl implements WmsScrapIssueService {

	@Autowired
	WmsScrapIssueDao dao;
	...
	public void addR(JSONObject json) throws Exception {
		...
		r.put("materialNo", json.getString("materialNo"));
		List<JSONObject> rList = dao.listR(json);
		if (rList != null && rList.size() > 0) {
			throw new Exception("该物料已录入");
		}
		dao.addR(json);
	}
	...
	public void guoZhang(JSONObject json) throws Exception {
		...
		for (JSONObject postingData : postingDataList) {
			...
			// 交易类型
			inventoryTransaction.put("transactionType", "051");
			// 操作员
			inventoryTransaction.put("userId", userId);
			// 交易代码
			inventoryTransaction.put("transactionCode", "报废出库");
			// 发出库位
