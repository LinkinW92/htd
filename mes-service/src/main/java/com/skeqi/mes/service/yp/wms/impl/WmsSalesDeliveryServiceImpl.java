...
import com.skeqi.mes.mapper.yp.wms.WmsSalesDeliveryDao;
...
import com.skeqi.mes.service.yp.wms.WmsSalesDeliveryService;
 ...
 */
@Service
public class WmsSalesDeliveryServiceImpl implements WmsSalesDeliveryService {

	@Autowired
	WmsSalesDeliveryDao dao;
	...
	public void add(JSONObject json) throws Exception {
		...
		// 查询销售订单R
		List<JSONObject> salesOrderRList = dao.findSalesOrderR(orderNumber);
		for (JSONObject salesOrderR : salesOrderRList) {
			...
			r.put("demandQuantity", salesOrderR.getString("quantity"));
	 ...
	 */
	@Override
	public List<JSONObject> findSalesOrderH(String orderNumber) {
		return dao.findSalesOrderH(orderNumber);
	}
	...
	public void guoZhang(JSONObject json) throws Exception {
		...
		for (JSONObject postingData : postingDataList) {
			...
			// 交易类型
			inventoryTransaction.put("transactionType", "031");
			// 操作员
			inventoryTransaction.put("userId", userId);
			// 交易代码
			inventoryTransaction.put("transactionCode", "销售出库");
			// 发出库位
