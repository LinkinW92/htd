...
import com.skeqi.mes.mapper.yp.equipment.maintain.EquipmentMaintainConfigDao;
 ...
 */
@Service
public class EquipmentMaintainConfigServiceImpl implements EquipmentMaintainConfigService {

	@Autowired
	EquipmentMaintainConfigDao dao;
	...
	public void add(JSONObject json) throws Exception {
		...
		if (dao.findConfigByEquipmentId(equipmentId) > 0) {
			throw new Exception("已选设备已存在保养配置");
		}

		// 查询名称重复数量
		if (dao.findNameCount(name, null) > 0) {
			throw new Exception("保养名重复");
		}

		// 查询编号重复数量
		if (dao.findCodeCount(code, null) > 0) {
			throw new Exception("保养编号重复");
		}
