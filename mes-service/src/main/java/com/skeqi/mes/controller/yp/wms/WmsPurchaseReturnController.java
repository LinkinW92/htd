 ...
 */
@RestController
@RequestMapping("/api/wms/wmsPurchaseReturn")
public class WmsPurchaseReturnController {
	
	@Autowired
	WmsPurchaseReturnService service;
	 ...
	 * @return
	 */
	@RequestMapping("/findPurchaseOrderH")
	public Rjson findPurchaseOrderH(HttpServletRequest request) {
		try {
			String listNo = EqualsUtil.string(request, "listNo", "单据号", false);
			
			List<JSONObject> list = service.findPurchaseOrderH(listNo);
