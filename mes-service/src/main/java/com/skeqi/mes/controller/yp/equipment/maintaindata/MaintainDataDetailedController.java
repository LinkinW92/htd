 ...
 */
@RestController
@RequestMapping("/api/equipment/maintainDataDetailed")
public class MaintainDataDetailedController {

	@Autowired
	MaintainDataDetailedService service;
	...
	public Rjson list(HttpServletRequest request) {
		try {
			int checkDataId = EqualsUtil.integer(request, "checkDataId", "保养记录", true);
	...
	public Rjson update(HttpServletRequest request) {
		try {
			...
			int result = EqualsUtil.integer(request, "result", "保养结果", true);
