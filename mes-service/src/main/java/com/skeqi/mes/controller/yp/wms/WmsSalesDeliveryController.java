 ...
 */
@RestController
@RequestMapping("/api/wms/wmsSalesDelivery")
public class WmsSalesDeliveryController {
	
	@Autowired
	WmsSalesDeliveryService service;
	...
	public Rjson add(HttpServletRequest request) {
		try {
			...
			String orderNumber = EqualsUtil.string(request, "orderNumber", "订单号", true);
	 ...
	 * @return
	 */
	@RequestMapping("/findSalesOrderH")
	public Rjson findSalesOrderH(HttpServletRequest request) {
		try {
			String listNo = EqualsUtil.string(request, "listNo", "单据号", false);
			
			List<JSONObject> list = service.findSalesOrderH(listNo);
