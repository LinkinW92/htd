package com.skeqi.mes.dingtalk.service;
...
import com.skeqi.mes.dingtalk.config.UrlConstant;
import com.skeqi.mes.dingtalk.domain.ServiceResult;
import com.skeqi.mes.dingtalk.util.FileUtil;
...
public class DingtalkTokenService {
    private static final Logger log = LoggerFactory.getLogger(DingtalkTokenService.class);
    /**
     * 缓存时间：一小时50分钟
     */
    ...
    @Autowired
    public DingtalkTokenService(AppConfig appConfig) {
    ...
    public ServiceResult<String> getAccessToken() {
        ...
        try {
        	// 从持久化存储中读取
            accessToken = getFromCache("accessToken", "access_token");
            if (accessToken != null) {
