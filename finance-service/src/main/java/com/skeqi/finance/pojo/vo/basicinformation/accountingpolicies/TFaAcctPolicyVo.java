package com.skeqi.finance.pojo.vo.basicinformation.accountingpolicies;

import com.skeqi.common.annotation.Excel;
import java.util.Date;

import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Data;


/**
 * 会计政策视图对象 t_fa_acct_policy
 *
 * @author toms
 * @date 2021-07-09
 */
@Data
@ApiModel("会计政策视图对象")
public class TFaAcctPolicyVo {

	private static final long serialVersionUID = 1L;

	/** 内码 */
	@ApiModelProperty("内码")
	private Integer fAcctpolicyId;

	/** 名称 */
	@Excel(name = "名称")
	@ApiModelProperty("名称")
	private String fName;

	/** 编码 */
	@Excel(name = "编码")
	@ApiModelProperty("编码")
	private String fNumber;

	/** 描述 */
	@Excel(name = "描述")
	@ApiModelProperty("描述")
	private String fDescription;

	/** 币别 */
	@Excel(name = "币别")
	@ApiModelProperty("币别")
	private Integer fCurrencyId;

	/** 币别名称 */
	@Excel(name = "币别名称")
	@ApiModelProperty("币别名称")
	private String fCurrencyName;

	/** 币别编码 */
	@Excel(name = "币别编码")
	@ApiModelProperty("币别编码")
	private String fCurrencyNumber;

	/** 会计日历 */
	@Excel(name = "会计日历")
	@ApiModelProperty("会计日历")
	private Integer fAcctcalendarId;

	/** 会计日历名称 */
	@Excel(name = "会计日历名称")
	@ApiModelProperty("会计日历名称")
	private String fAcctcalendarName;

	/** 会计日历编码 */
	@Excel(name = "会计日历编码")
	@ApiModelProperty("会计日历编码")
	private String fAcctcalendarNumber;

	/** 会计要素 */
	@Excel(name = "会计要素")
	@ApiModelProperty("会计要素")
	private Integer fAcctgroupId;

	/** 会计要素名称 */
	@Excel(name = "会计要素名称")
	@ApiModelProperty("会计要素名称")
	private String fAcctgroupName;

	/** 会计要素编码 */
	@Excel(name = "会计要素编码")
	@ApiModelProperty("会计要素编码")
	private String fAcctgroupNumber;

	/** 汇率类型 */
	@Excel(name = "汇率类型")
	@ApiModelProperty("汇率类型")
	private Integer fRatetypeId;

	/** 汇率类型名称 */
	@Excel(name = "汇率类型名称")
	@ApiModelProperty("汇率类型名称")
	private String fRatetypeName;

	/** 汇率类型编码 */
	@Excel(name = "汇率类型编码")
	@ApiModelProperty("汇率类型编码")
	private String fRatetypeNumber;

	/** 存货核算按费用项目明细核算默认0 */
	@Excel(name = "存货核算按费用项目明细核算默认0")
	@ApiModelProperty("存货核算按费用项目明细核算默认0")
	private String fIscalbyexpItem;

	/** 成本以含税金额进行核算 */
	@Excel(name = "成本以含税金额进行核算")
	@ApiModelProperty("成本以含税金额进行核算")
	private String fIstaxAmount;

	/** 启用实际成本核算 默认0 */
	@Excel(name = "启用实际成本核算 默认0")
	@ApiModelProperty("启用实际成本核算 默认0")
	private String fIscalbyactualCost;

	/** 是否默认 0：非默认，默认 1： */
	@Excel(name = "是否默认 0：非默认，默认 1：")
	@ApiModelProperty("是否默认 0：非默认，默认 1：")
	private String fIsdefault;

	/** 创建组织 */
	@Excel(name = "创建组织")
	@ApiModelProperty("创建组织")
	private Integer fCreateOrgid;

	/** 创建组织名称 */
	@Excel(name = "创建组织名称")
	@ApiModelProperty("创建组织名称")
	private String fCreateOrgName;

	/** 创建组织编码 */
	@Excel(name = "创建组织编码")
	@ApiModelProperty("创建组织编码")
	private String fCreateOrgNumber;

	/** 创建人 */
	@Excel(name = "创建人")
	@ApiModelProperty("创建人")
	private Integer fCreatorid;

	/** 创建日期 */
	@Excel(name = "创建日期" , width = 30, dateFormat = "yyyy-MM-dd")
	@ApiModelProperty("创建日期")
	private Date fCreateDate;

	/** 使用组织 */
	@Excel(name = "使用组织")
	@ApiModelProperty("使用组织")
	private Integer fUseOrgid;

	/** 修改人 */
	@Excel(name = "修改人")
	@ApiModelProperty("修改人")
	private Integer fModifierid;

	/** 修改日期 */
	@Excel(name = "修改日期" , width = 30, dateFormat = "yyyy-MM-dd")
	@ApiModelProperty("修改日期")
	private Date fModifyDate;

	/** 数据状态 */
	@Excel(name = "数据状态")
	@ApiModelProperty("数据状态")
	private String fDocumentStatus;

	/** 审核人 */
	@Excel(name = "审核人")
	@ApiModelProperty("审核人")
	private Integer fAuditorid;

	/** 审核日期 */
	@Excel(name = "审核日期" , width = 30, dateFormat = "yyyy-MM-dd")
	@ApiModelProperty("审核日期")
	private Date fAuditDate;

	/** 禁用状态 */
	@Excel(name = "禁用状态")
	@ApiModelProperty("禁用状态")
	private String fForbidStatus;

	/** 禁用人 */
	@Excel(name = "禁用人")
	@ApiModelProperty("禁用人")
	private Integer fForbidderid;

	/** 禁用日期 */
	@Excel(name = "禁用日期" , width = 30, dateFormat = "yyyy-MM-dd")
	@ApiModelProperty("禁用日期")
	private Date fForbidDate;

	/** 是否系统预设1 系统预设0 非系统预设默认0 */
	@Excel(name = "是否系统预设1 系统预设0 非系统预设默认0")
	@ApiModelProperty("是否系统预设1 系统预设0 非系统预设默认0")
	private Integer fIssysPreset;

	/** 组织隔离字段 */
	@Excel(name = "组织隔离字段")
	@ApiModelProperty("组织隔离字段")
	private Integer fMasterId;


}
